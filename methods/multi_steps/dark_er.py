...
class Dark_ER(Finetune_IL):
    ...
    def _epoch_train(self, model, train_loader, optimizer, scheduler, task_begin=None, task_end=None, task_id=None):
        ...
        for _, inputs, targets in train_loader:
            ...
            if task_id > 0:                
                buf_inputs, _, buf_soft_targets = self._memory_bank.get_memory_reservoir()
                buf_inputs, buf_soft_targets = buf_inputs.cuda(), buf_soft_targets.cuda()
                kl_loss = mse_loss(model(buf_inputs)[0].float(), buf_soft_targets.float())
                kl_losses += kl_loss.item()

                buf_inputs, buf_targets, _ = self._memory_bank.get_memory_reservoir()
                buf_inputs, buf_targets = buf_inputs.cuda(), buf_targets.cuda()
                buf_ce_loss = cross_entropy(model(buf_inputs)[0], buf_targets)
                buf_ce_losses += buf_ce_loss.item()

                loss = ce_loss + self._alpha*kl_loss + self._beta*buf_ce_loss
            else:
                loss = ce_loss
        ...
        train_loss = ['Loss', losses/len(train_loader), 'Loss_ce', ce_losses/len(train_loader),
            'Loss_kl', kl_losses/len(train_loader), 'Loss_buf_ce', buf_ce_losses/len(train_loader)]
