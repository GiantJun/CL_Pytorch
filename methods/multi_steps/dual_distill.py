...
def add_special_args(parser: ArgumentParser) -> ArgumentParser:
    parser.add_argument('--T1', type=float, default=None, help='tempreture apply to the output logits befor softmax')
    parser.add_argument('--T2', type=float, default=None, help='tempreture apply to the output logits befor softmax')
    parser.add_argument('--lamda1', type=float, default=None, help='hyperparameter for dual distill')
    parser.add_argument('--lamda2', type=float, default=None, help='hyperparameter for dual distill')
    return parser
...
class Dual_Distill(Finetune_IL):
    ...
    def __init__(self, logger, config):
        ...
        self._T1 = config.T1
        self._T2 = config.T2
        self._lamda1 = config.lamda1
        self._lamda2 = config.lamda2
    ...
    def prepare_task_data(self, data_manager):
        ...
        self._new_task_train_dataset = data_manager.get_dataset(indices=np.arange(self._known_classes, self._total_classes),
                    source='train', mode='train')
        self._new_task_test_dataset = data_manager.get_dataset(indices=np.arange(self._known_classes, self._total_classes),
        ...
        self._new_task_train_loader = DataLoader(self._new_task_train_dataset, batch_size=self._batch_size, shuffle=True, num_workers=self._num_workers)

        self._test_loader = DataLoader(self._test_dataset, batch_size=self._batch_size, shuffle=False, num_workers=self._num_workers)
        self._new_task_test_loader = DataLoader(self._new_task_test_dataset, batch_size=self._batch_size, shuffle=True, num_workers=self._num_workers)
    ...
    def incremental_train(self):
        ...
        if self._cur_task == 0:
            ...
            self._is_stage_two = False
            self._logger.info('Stage 1: Training expert model with current task data...')
            optimizer = self._get_optimizer(filter(lambda p: p.requires_grad, self._expert_network.parameters()), self._config, False)
            scheduler = self._get_scheduler(optimizer, self._config, False)
            self._expert_network = self._train_model(self._expert_network, self._new_task_train_loader, self._new_task_test_loader, optimizer, scheduler,
            ...
            self._logger.info('Stage 2: Training updated model with current and previous task data...')
    ...
    def _epoch_train(self, model, train_loader, optimizer, scheduler, task_begin=None, task_end=None, task_id=None):
        ...
        for _, inputs, targets in train_loader:
            ...
            if task_id == 0:
                ...
                origin_kd_loss = self._KD_loss(logits[:,:task_begin], self._origin_network(inputs)[0], self._T1)
                origin_kd_losses += origin_kd_loss.item()

                expert_kd_loss = self._KD_loss(logits[:, task_begin:], self._expert_network(inputs)[0], self._T2)
