...
class GEM(Finetune_IL):
    ...
    def after_task(self):
        ...
        self._logger.info('Exemplar size: {}'.format(self.exemplar_size))
    ...
    def incremental_train(self, data_manager):
        ...
        if self._cur_task>0:
            previous_dataset= data_manager.get_dataset(indices=[],source='train',
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                dotprod = torch.mm(G[:, self._cur_task].unsqueeze(0), G[:, :self._cur_task])
                ...
                if(dotprod < 0).sum() >0:
                    ...
                    for params in self._network.parameters():
                        if params is not None:
                            ...
                            endpt = sum(grad_numels[:j+1])
                            params.grad.data.copy_(new_grad[stpt:endpt].contiguous().view(params.grad.data.size()))
                            j += 1
